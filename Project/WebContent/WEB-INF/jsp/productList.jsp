...
<body>
	...
	<div class="productList-area shadow-lg p-3 mb-5 bg-light">
		<h2 class="productList-top">商品マスタ一覧</h2>
		...
		<form action="ProductSearchResult" method="post">
			<div class="product-search-area">
				<div class="form-group row">
					...
					<input type="text" name="search_productName" class="form-control col-sm-7" id="name" placeholder="部分一致">
				</div>
				<div class="form-group row">
					<label for="cost" class="col-sm-3 text-center">価格</label>
					<input type="text" name="low_productCost" class="form-control col-sm-2" id="cost">
					<p class="col-sm-1 text-right">円</p>
					<p class="col-sm-1">～</p>
					<input type="text" name="high_productCost" class="form-control col-sm-2" id="cost">
					<p class="col-sm-1 text-right">円</p>
				</div>
		...
		<div class="text-center"> （${productAll}件中${productPageTop}～${productPageBottom}件目）
		<div class="row">
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination justify-content-center">
				...
				<c:if test="${!(pageNum == pageMax) && !(productAll == 0)}">
					<li class="page-item"><a class="page-link" href="ProductList?page_num=${pageNum + 1}">${pageNum + 1}</a></li>
				</c:if>
