...
<body>
	...
	<div class="userList-area shadow-lg p-3 mb-5 bg-light">
		<h2 class="userList-top">ユーザ一覧</h2>
		<form action="UserSearchResult">
			<div class="user-search-area">
				<div class="form-group row">
					<label for="loginId" class="col-sm-3 text-center">ログインID</label>
					<input type="text" name="search_loginId" class="form-control col-sm-7" id="loginId" placeholder="完全一致">
				</div>
				<div class="form-group row">
					<label for="userName" class="col-sm-3 text-center">ユーザ名</label>
					<input type="text" name="search_userName" class="form-control col-sm-7" id="userName" placeholder="部分一致">
				</div>
				<div class="form-group row">
					<label for="birthdate" class="col-sm-3 text-center">生年月日</label>
					<input type="date" name="start_Birthdate" id="birthdate" class="form-control col-sm-3">
					<p class="col-sm-1 text-center">～</p>
					<input type="date" name="end_Birthdate" id="birthdate" class="form-control col-sm-3">
				</div>
		...
		<div class="text-center"> （${userAll}件中${userPageTop}～${userPageBottom}件目）
		<div class="row">
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination justify-content-center">
				...
				<c:if test="${!(pageNum == 1)}">
					<li class="page-item">
						<a class="page-link" href="UserList"><<</a>
					</li>
				</c:if>

				<c:if test="${pageNum == pageMax && pageMax >= 3}">
					<li class="page-item"><a class="page-link" href="UserList?page_num=${pageNum - 2}">${pageNum - 2}</a></li>
				</c:if>
				<c:if test="${!(pageNum == 1)}">
					<li class="page-item"><a class="page-link" href="UserList?page_num=${pageNum - 1}">${pageNum - 1}</a></li>
				</c:if>
				...
				<c:if test="${!(pageNum == pageMax) && !(userAll == 0)}">
					<li class="page-item"><a class="page-link" href="UserList?page_num=${pageNum + 1}">${pageNum + 1}</a></li>
				</c:if>
				...
				<c:if test="${!(pageNum == pageMax) && !(userAll == 0)}">
					<li class="page-item">
						<a class="page-link" href="UserList?page_num=${pageMax}">>></a>
					</li>
				</c:if>
			</ul>
		</nav>
